...
const library = {
  ...
  validateOutputPath: function (options, operatingSystem) {
    ...
    if (options[operatingSystem].outputPath) {
      if (process.platform === 'win32') {
        options[
          operatingSystem
        ].outputPath = this.resolveWindowsEnvironmentVariables(
          options[operatingSystem].outputPath
        );
      } else {
        options[operatingSystem].outputPath = this.resolveTilde(
          options[operatingSystem].outputPath
        );
      }
      if (
        ...
        this.throwError(
          options,
          'Optional ' +
            operatingSystem.toUpperCase() +
            ' outputPath must exist and be a folder. Defaulting to desktop.'
        );
    ...
    const fileName =
      options[operatingSystem].name ||
      path.parse(options[operatingSystem].filePath).name ||
      'Root';
    ...
    options[operatingSystem].outputPath = path.join(
      options[operatingSystem].outputPath,
  ...
  validateLinuxFilePath: function (options) {
    ...
    if (
      ...
      this.throwError(
        options,
        'LINUX filePath does not exist: ' + options.linux.filePath
      );
      delete options.linux;
    } else if (
      type &&
      ...
      this.throwError(
        options,
        'LINUX filePath directory must exist and be a folder: ' +
          options.linux.filePath
      );
      delete options.linux;
    } else if (
      type &&
      type === 'Link' &&
      (!options.linux.filePath || typeof options.linux.filePath !== 'string')
    ) {
      this.throwError(
        options,
        'LINUX filePath url must exist a string: ' + options.linux.filePath
      );
      delete options.linux;
    }
  ...
  validateLinuxOptions: function (options) {
    ...
    if (options.linux.type && !validTypes.includes(options.linux.type)) {
      this.throwError(
        options,
        'Optional LINUX type must be "Application", "Link", or "Directory". Defaulting to "Application".'
      );
    ...
    if (options.linux.icon) {
      ...
      if (!iconPath.endsWith('.png') && !iconPath.endsWith('.icns')) {
        this.throwError(
          options,
          'Optional LINUX icon should probably be a PNG file.'
        );
      }
  ...
  validateWindowsFilePath: function (options) {
    ...
    if (options.windows.filePath) {
      options.windows.filePath = this.resolveWindowsEnvironmentVariables(
    ...
    if (
      ...
      this.throwError(
        options,
        'WINDOWS filePath does not exist: ' + options.windows.filePath
      );
      delete options.windows;
    }
  ...
  validateWindowsOptions: function (options) {
    ...
    if (
      options.windows.windowMode &&
      !validWindowModes.includes(options.windows.windowMode)
    ) {
      this.throwError(
        options,
        'Optional WINDOWS windowMode must be "normal", "maximized", or "minimized". Defaulting to "normal".'
      );
    ...
    if (options.windows.icon) {
      let iconPath = this.resolveWindowsEnvironmentVariables(
        options.windows.icon
      );
      ...
      if (!RegExp(iconPattern).test(iconPath)) {
        this.throwError(
          options,
          "Optional WINDOWS icon must be a ICO, EXE, or DLL file. It may be followed by a comma and icon index value, like: 'C:\\file.exe,0'"
        );
      }
  ...
  validateOSXFilePath: function (options) {
    ...
    if (
      ...
      this.throwError(
        options,
        'OSX filePath does not exist: ' + options.osx.filePath
      );
      delete options.osx;
    }
  ...
  makeLinuxShortcut: function (options) {
    ...
    if (success && options.linux.chmod) {
      try {
        ...
        this.throwError(
          options,
          'ERROR attempting to change permisions of ' +
            options.linux.outputPath,
          error
        );
      }
  ...
  makeWindowsShortcut: function (options) {
    ...
    let cwd =  path.join(BASE_URI, 'bin'); //''; //try to fix start-in not existing, and leading to no log file....
  ...
  makeOSXShortcut: function (options) {
    ...
    if (
      options.osx.overwrite ||
      (!options.osx.overwrite && !fs.existsSync(options.osx.outputPath))
    ) {
      let command = [
        'ln',
        overwrite,
      ...
      this.throwError(
        options,
        'Could not create OSX shortcut because matching outputPath already exists and overwrite is false.'
      );
    }
...
export default CreateDesktopShortcut;
